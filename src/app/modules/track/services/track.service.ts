...
export class TrackService {
  ...
  constructor(private http: HttpClient) {
    ...
    this.limit = '&limit=';
    this.microServiceUrl = 'localhost:8098/api/v1/';
  ...
  updateAddedTrack(track: Track) {
    return this.http
    .put(this.microServiceUrl + 'tracks', track, { observe: 'response' })
    .pipe(catchError(this.handleError));
  }

  deleteTrackFromPlayList(trackId: string) {
    console.log(this.microServiceUrl + 'tracks' + trackId);
    return this.http.delete(this.microServiceUrl + 'tracks' + trackId, { observe: 'response' })
    .pipe(catchError(this.handleError));
  }
